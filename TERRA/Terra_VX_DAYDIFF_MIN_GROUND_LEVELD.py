...
# Load data
chunk = np.loadtxt(r"C:\github\CzechFOI\TERRA\Terra_VX_DAYDIFF_MIN_GROUND_LEVELD.csv", comments='#', delimiter=',', skiprows=1)
DATA = np.array(chunk)
Ys = DATA[:, 0]
Xs = DATA[:, 1]
Zs = DATA[:, 2]
...
fig.update_layout(
    scene=dict(
        ...
        zaxis_title='Number of Days until Death after VXD1 minimum ground leveled',
...
# Export to HTML
fig.write_html(r"C:\github\CzechFOI\3D Plot Results\Terra_VX_DAYDIFF_MIN_GROUND_LEVELD.html")
