...
# Load data
chunk = np.loadtxt(r"C:\github\CzechFOI\TERRA\Terra_VX_DAYDIFF.csv", comments='#', delimiter=',', skiprows=1)
DATA = np.array(chunk)
Ys = DATA[:, 1]
Xs = DATA[:, 2]
Zs = DATA[:, 3]
...
fig.update_layout(
    scene=dict(
        ...
        zaxis_title='Number of Days until Death after VXD1',
...
# Export to HTML
fig.write_html(r"C:\github\CzechFOI\3D Plot Results\Terra_VX_DAYDIFF.html")
